...
char * CreateResult(uint32_t A0, uint32_t B0, uint32_t C0, uint32_t D0) {
...
char * md5(const char *message) {
	...
	memcpy((uint8_t*)buffer + elements * sizeof(uint32_t) - 8, &bitLength,sizeof(uint64_t));
	...
	char *result = CreateResult(A0, B0, C0, D0);
	return result;
}
